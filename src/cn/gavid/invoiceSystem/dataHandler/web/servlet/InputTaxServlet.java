 ...
 *
 */
public class InputTaxServlet extends BaseServlet {
	...
	public void getAmountJson(HttpServletRequest request, HttpServletResponse response)
		 ...
		 * 向客户端发送json串
		 */
		Map<String, BigDecimal> amountTaxDate = inputTaxService.amountTaxDate();
	...
	public void getTotalJson(HttpServletRequest request, HttpServletResponse response)
		 ...
		 * 向客户端发送json串
		 */
		Map<String, BigDecimal> includeTotalDate = inputTaxService.includeTaxTotalDate();
	 ...
	 * @throws IOException
	 */
	public void getMonthInData(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		response.setContentType("text/html;charset=utf-8");
		/*
		 * 向客户端发送json串
		...
		for(int i = 1;i<=12;i++){
			...
			for(Integer integer : monthInData.keySet()){
				if(integer.intValue() == i){
					monthvalues.add(monthInData.get(integer).toString());
					flag = false;
					break;
				}
			}
	 ...
	 * @throws IOException
	 */
	public void getWeekInData(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		response.setContentType("text/html;charset=utf-8");
		/*
		 * 向客户端发送json串
		...
		for(int i = 1;i<=52;i++){
			...
			for(Integer integer : weekInData.keySet()){
				if(integer.intValue() == i){
					weekvalues.add(weekInData.get(integer).toString());
					flag = false;
					break;
				}
			}
	 ...
	 * @throws IOException
	 */
	public void getQuarterInData(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		response.setContentType("text/html;charset=utf-8");
		/*
		 * 向客户端发送json串
